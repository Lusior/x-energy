(function ($) {
    ...
    BootstrapTable.prototype.initFixedColumns = function () {
        this.$fixedBody = $([
            '<div class="fixed-table-column" style="position: absolute; background-color: #fff; border-right:2px solid #ddd;">',
            '<table>',
            '<thead></thead>',
            '<tbody style="display: none; box-sizing: border-box;overflow: hidden;"></tbody>', 
//            这个display:none以后调整
            '</table>',
            '</div>'].join(''));
    ...
    BootstrapTable.prototype.initBody = function () {
        ...
        this.$body.find('> tr[data-index]').each(function (index) {
            ...
            $tr.html('');
            //qqhr
            if(that.options.pageNumber == 1) {//如果是第一页
